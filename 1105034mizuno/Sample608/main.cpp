...
int main()
{
	map<string, int> score;//mapコンテナのインスタンス作成

	score["Tom"] = 100;//キー:Tom 値:100のペアをscoreに追加
	score["Bob"] = 80; //キー:Bob 値:80のペアをscoreに追加
	score["Mike"] = 76;//キー:Mike 値:76のペアをscoreに追加
	score.emplace("David", 90);//キー:David 値:90のペアをscoreに追加
	score.insert({ "John",70 });//キー:John 値:70のペアをscoreに追加
	...
	cout << "Tomの点数は" << score["Tom"] << endl;
	cout << "Bobの点数は" << score["Bob"] << endl;
	if (score.count("Mike"))//Mikeのキーの存在をチェック
	{
		cout << "Mikeの点数は" << score["Mike"] << endl;
	}
	auto it = score.find("David");//Davidキーの場所をイテレータで取得
